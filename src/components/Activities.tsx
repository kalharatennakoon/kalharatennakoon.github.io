    ...
    <section id="activities" className="py-20 bg-[var(--bg-secondary)] dark:bg-[var(--bg-primary)]">
      <div className="max-w-6xl mx-auto px-8">
        ...
        <div className="flex items-center justify-center gap-4 max-w-3xl mx-auto mb-12 px-8 py-4 bg-gradient-to-br from-[rgba(102,126,234,0.08)] to-[rgba(118,75,162,0.08)] rounded-full border-2 border-[rgba(102,126,234,0.2)] dark:from-[rgba(102,126,234,0.12)] dark:to-[rgba(118,75,162,0.12)] dark:border-[rgba(102,126,234,0.3)]">
          <FaHandsHelping className="text-2xl flex-shrink-0 text-[#667eea]" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-5xl mx-auto">
          {activities.map((activity) => (
            <div
              key={activity.title}
              className="bg-[var(--card-bg)] p-6 rounded-2xl shadow-[0_4px_15px_var(--shadow)] border border-[var(--border-color)] hover:border-[rgba(102,126,234,0.4)] hover:-translate-y-1 hover:shadow-[0_8px_25px_rgba(102,126,234,0.15)] transition-all duration-300"
            >
              <div className="flex items-start gap-4">
                <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-[#667eea] to-[#764ba2] flex items-center justify-center text-white text-xl flex-shrink-0">
                  {activity.icon}
                </div>
                <div className="flex-1 min-w-0">
                  <h3 className="text-lg font-bold text-[var(--text-primary)] mb-0.5 leading-snug">{activity.title}</h3>
                  <div className="flex flex-wrap items-center gap-x-2 gap-y-1 mb-2">
                    <span className="text-sm font-medium text-[#667eea]">{activity.role}</span>
                  ...
                  {activity.stats && (
                    <div className="flex flex-wrap gap-2 mt-3">
                      {activity.stats.map((stat) => (
                        <span
                          key={stat}
                          className="px-3 py-1 bg-[rgba(102,126,234,0.1)] text-[#667eea] rounded-full text-xs font-semibold border border-[rgba(102,126,234,0.2)]"
                        >
                          {stat}
                        </span>
                      ))}
