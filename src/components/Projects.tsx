  ...
  return (
    <section id="projects" className="py-20 bg-[var(--bg-primary)] dark:bg-[var(--bg-secondary)]">
      <div className="max-w-6xl mx-auto px-8">
        <h2 className="text-5xl mb-8 text-center text-[var(--text-primary)] font-bold bg-gradient-to-r from-[var(--color-primary)] to-[var(--color-primary)] bg-clip-text text-transparent">
          Projects
        </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {projects.map((project) => (
            <div
              key={project.title}
              className="group bg-[var(--card-bg)] p-8 rounded-2xl shadow-[0_4px_15px_var(--shadow)] transition-all duration-300 hover:-translate-y-3 hover:shadow-[0_12px_35px_rgba(30,58,138,0.2)] border border-[var(--border-color)] hover:border-[rgba(30,58,138,0.4)] backdrop-blur-sm relative overflow-hidden"
            >
              ...
              <h3 className="text-2xl mb-4 text-[var(--color-primary)] font-bold relative z-10 group-hover:text-[var(--color-primary)] transition-colors">
                {project.title}
              </h3>
              ...
              <div className="flex flex-wrap gap-2 mb-4 relative z-10">
                {project.technologies.map((tech) => (
                  <span
                    key={tech}
                    className="px-3 py-1 bg-gradient-to-r from-[rgba(30,58,138,0.1)] to-[rgba(23,37,84,0.1)] rounded-full text-sm text-[var(--text-primary)] font-medium border border-[rgba(30,58,138,0.2)] hover:border-[rgba(30,58,138,0.4)] transition-colors"
                  >
                    {tech}
                  </span>
                ))}
              ...
              <div className="flex gap-4 relative z-10">
                <a
                  ...
                  className="text-[var(--color-primary)] font-semibold hover:text-[var(--color-primary)] transition-colors inline-flex items-center gap-2 group-hover:gap-3 transition-all"
                >
                  <FaGithub /> GitHub
                </a>
              </div>
